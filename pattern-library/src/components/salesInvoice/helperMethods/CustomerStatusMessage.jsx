...
const CustomerStatusMessage = ({ formData }) => {
        ...
        return {
          message: (
            ...
            >
              <i className="bi bi-exclamation-triangle-fill me-2"></i>
              <strong>
                Unable to create sales invoice. Customer has unsettled invoices
                within credit duration.
              </strong>
            </div>
          ),
          isError: true,
  ...
  const status = useMemo(() => {
    return formData?.selectedCustomer
      ? getStatus(formData, unSettledInvoices)
      : { message: null, isError: true };
  }, [formData, unSettledInvoices, getStatus]);
